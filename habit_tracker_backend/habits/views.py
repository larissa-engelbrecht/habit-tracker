...
def period_completion_history(request, habit_id):
    ...
    try:
        ...
        response_data = {
            ...
            'current_period': habit.get_current_progress()
        }
...
def stats_data(request):
    ...
    try:
        ...
        # Sort streaks by current streak length (descending)
        streak_data_list.sort(key=lambda x: x['currentStreak'], reverse=True)

        # Determine the single best streak for the overview card
        best_streak = None
        ...
        # Improved logic for finding best and worst performers
        best_performer = None
        ...
        response_data = {
            'overallStats': {
                ...
                'bestStreak': best_streak, # Frontend expects this
                'bestPerformer': best_performer,
                'worstPerformer': worst_performer,
                'daysActive': days_active
            },
            'habitStats': habit_stats_list,
            'streakData': streak_data_list # Frontend expects this for the streaks tab
        }
